...
from albumentations import HorizontalFlip, VerticalFlip, ChannelShuffle, CenterCrop, Crop, Rotate
...
def load_data(path, split=0.1):
    ...
    X = sorted(glob(os.path.join(path, "train", "Images", "*.jpg")))
    Y = sorted(glob(os.path.join(path, "train", "labels", "*.png")))
    ...
    for x, y in tqdm(zip(images, masks), total=len(images)):
        ...
        for i, m in zip(X, Y):
            if index == 0:
                ...
                scale = int(x.shape[0] * scale_h/100)

                i = cv2.resize(i, (scale, scale))
                m = cv2.resize(m, (scale, scale))

                i = i[0:H, 0:W, :]
                m = m[0:H, 0:W]

                if i.shape[0] >= H and i.shape[1] >= W:
                ...
                num_crops_h = i.shape[0]//H
                num_crops_w = i.shape[1]//W

                for xh in range(0, num_crops_h, 3):
                    for xw in range(0, num_crops_w, 3):
                        ci = i[xh*H:(xh+1)*H, xw*W:(xw+1)*W: ]
                        cm = m[xh*H:(xh+1)*H, xw*W:(xw+1)*W ]
                        ...
                        cv2.imwrite(image_path, ci)
                        cv2.imwrite(mask_path, cm)
